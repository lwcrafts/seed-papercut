// 纸雕蒙版拓扑修复（issue 13，移植自 .scratch/spikes/topology/src/topology.ts + raster.ts，
// 参数按票 09 作者拍板：桥宽 3mm、缝隙红线 1mm 两段式、形态学开运算去毛刺）。
//
// 约定：px[i] = 1 白纸（保留），0 镂空。连通判定一律 4 邻接（激光切割语境：
// 对角点接触是零宽连接，8 邻接会漏报孤岛）；距离/最短路传播用 8 邻（几何度量）。
//
// 与 spike 的差异（矢量化迁移，票 09 拍板第 4/5 条）：
// - 加桥不再用像素方块膨胀，而是沿逃生路径扫掠圆头（capsule，矩形+圆头端），
//   矢量化后即自然成为矩形/圆头桥几何；
// - 加桥后再做一次缝隙闭合（红线 1mm），然后才交给矢量化；
// - 切割周长的展示值在矢量域精算（bake.mjs 内），本模块只保留像素域粗判指标。

import { createRequire } from 'node:module';
const require = createRequire(import.meta.url);
const ImageTracer = require('./vendor/imagetracer.cjs');

export { ImageTracer };

// ---------- 基础 ----------

export function inside(m, x, y) {
...
export function connectedComponents(m) {
  const { w, h, px } = m;
...
function paintCapsule(m, label, path, radius, newLabel, cells) {
  const { w, h } = m;
  for (const p of path) {
    const x = p % w;
    const y = (p / w) | 0;
 ...
 */
export function repairTopology(m, bridgeRadiusPx) {
  ...
  const islands = islandComps.map((c) => {
  ...
  while (remaining.size > 0) {
    ...
    for (const c of islandComps) {
      ...
      const own = groupLabels.get(find(c.id));
    ...
    const { from, esc } = pick;
    paintCapsule(m, label, esc.path, bridgeRadiusPx, esc.targetId, bridgeCells);
    const ra = find(from.id);
    const rb = find(esc.targetId);
    if (ra !== rb) {
      parent[ra] = rb;
      const merged = groupLabels.get(ra);
      for (const t of merged) groupLabels.get(rb).add(t);
...
function neighborDist(m, dist, x, y) {
  if (!inside(m, x, y)) return 0;
  const p = y * m.w + x;
  return m.px[p] === 1 ? 0 : dist[p];
}
 ...
 */
function isSlotCenterline(dist, px, w, h, x, y) {
  const p = y * w + x;
  const d = dist[p];
  ...
  for (const [dx, dy] of N8) {
    ...
    const ny = y + dy;
    let nd;
    if (!insideXY(w, h, nx, ny)) nd = 0; // 画布外按白纸
    else {
      const np = ny * w + nx;
 ...
 */
export function minGapPxRidge(m) {
  const { w, h, px } = m;
 ...
 * 返回迭代次数。
 */
export function healNarrowGaps(m, minGapPx, maxIter = 30) {
  const { w, h, px } = m;
  ...
  for (let it = 1; it <= maxIter; it++) {
    ...
    for (const [x, y] of bad) {
      for (const [dx, dy] of disc) {
        const nx = x + dx;
        const ny = y + dy;
        if (insideXY(w, h, nx, ny)) m.px[ny * w + nx] = 1; // 涂白（含原本就是白的，幂等）
      }
    }
  }
  return maxIter;
 ...
 * 不可切割）。pass 次数默认 2。
 */
export function median3(m, passes = 2) {
  const { w, h } = m;
  let cur = m.px;
