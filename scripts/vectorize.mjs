// 矢量化（issue 13）：加桥/修复后的二值蒙版 → imagetracerjs 闭合路径 → LayerSet pathD
// + 矢量域精算指标（切割周长、最小缝隙精算）。
//
...
import { ImageTracer } from './topology.mjs';
...
function quadLen(s) {
  const ctrlPoly = Math.hypot(s.x2 - s.x1, s.y2 - s.y1) + Math.hypot(s.x3 - s.x2, s.y3 - s.y2);
  const chord = Math.hypot(s.x3 - s.x1, s.y3 - s.y1);
  return (ctrlPoly + chord) / 2;
}

function segmentsLengthPx(segments) {
  let len = 0;
  ...
  const ex = last.type === 'Q' ? last.x3 : last.x2;
  const ey = last.type === 'Q' ? last.y3 : last.y2;
...
function segmentsToD(segments, scale, roundcoords) {
  ...
  for (const s of segments) {
    if (s.type === 'Q') {
      str += `Q ${r(s.x2)} ${r(s.y2)} ${r(s.x3)} ${r(s.y3)} `;
    } else {
      str += `L ${r(s.x2)} ${r(s.y2)} `;
    }
  }
 ...
 */
export function vectorizeMask(mask, { scale, pxPerMm, roundcoords = 1 }) {
  const { w, h, px } = mask;
  ...
  const tracedata = ImageTracer.imagedataToTracedata({ width: w, height: h, data }, { ...TRACER_OPTIONS, roundcoords });
